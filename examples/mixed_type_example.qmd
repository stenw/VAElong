...
A **Time-Parameterized CNN (TPCNN)** replaces fixed convolution kernels with
kernels generated by small MLPs from relative time offsets. This enables the
same architecture to handle both regularly and irregularly sampled time series.
Here we train a TPCNN-based VAE as a drop-in replacement for the LSTM VAE.
...
An **encoder-only Transformer** VAE uses multi-head self-attention to model
temporal dependencies. The encoder pools the attended sequence into a latent
distribution; the decoder broadcasts the sampled latent across all time
positions and refines via bidirectional self-attention. Attention masking
naturally handles missing time steps.
...
We compare all five models — **LSTM VAE**, **LMM**, **Seq2Seq LSTM**,
**TPCNN VAE**, and **Transformer VAE** — on the same validation set and
future time window ($t = 25 \dots 49$).

```{python}
#| label: tbl-comparison
#| tbl-cap: "Landmark prediction accuracy (future portion only) across all five models."
